...
public class InstanceController {
    ...
    private DataSource pushDataSource = new DataSource() {
        ...
        public String getData(PushService.PushClient client) throws Exception {
            ...
            try {
                ...
                Loggers.SRV_LOG.warn("PUSH-SERVICE: provider is not modified", e);
            }
    ...
    public String deregister(HttpServletRequest request) throws Exception {
        ...
        if (service == null) {
            Loggers.SRV_LOG.warn("remove instance from non-exist provider: {}", serviceName);
            return "ok";
        }
    ...
    public JSONObject detail(HttpServletRequest request) throws Exception {
        ...
        Service service = serviceManager.getService(namespaceId, serviceName);
        if (service == null) {
            throw new NacosException(NacosException.NOT_FOUND, "no provider " + serviceName + " found!");
        }
        ...
        if (ips == null || ips.isEmpty()) {
            throw new NacosException(NacosException.NOT_FOUND,
                "no ips found for cluster " + cluster + " in provider " + serviceName);
        }

        for (Instance instance : ips) {
            if (instance.getIp().equals(ip) && instance.getPort() == port) {
                JSONObject result = new JSONObject();
                result.put("provider", serviceName);
                result.put("ip", ip);
    ...
    public JSONObject beat(HttpServletRequest request) throws Exception {
        ...
        if (service == null) {
            throw new NacosException(NacosException.SERVER_ERROR, "provider not found: " + serviceName + "@" + namespaceId);
        }
    ...
    public JSONObject listWithHealthStatus(HttpServletRequest request) throws NacosException {
        ...
        if (service == null) {
            throw new NacosException(NacosException.NOT_FOUND, "provider: " + serviceName + " not found.");
        }
    ...
    public void checkIfDisabled(Service service) throws Exception {
        if (!service.getEnabled()) {
            throw new Exception("provider is disabled now.");
        }
    }
    ...
    public JSONObject doSrvIPXT(String namespaceId, String serviceName, String agent, String clusters, String clientIP, int udpPort,
        ...
        if (service == null) {
            throw new NacosException(NacosException.NOT_FOUND, "provider not found: " + serviceName);
        }
        ...
        if (CollectionUtils.isEmpty(srvedIPs)) {
            ...
            if (Loggers.DEBUG_LOG.isDebugEnabled()) {
                Loggers.DEBUG_LOG.debug("no instance to serve for provider: " + serviceName);
            }
        ...
        if ((float) ipMap.get(Boolean.TRUE).size() / srvedIPs.size() <= threshold) {
            ...
            Loggers.SRV_LOG.warn("protect threshold reached, return all ips, provider: {}", serviceName);
            if (isCheck) {
