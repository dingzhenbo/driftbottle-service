...
public class ServiceManager implements RecordListener<Service> {
    ...
    public void init() {
        ...
        try {
            Loggers.SRV_LOG.info("listen for provider meta change");
            consistencyService.listen(KeyBuilder.SERVICE_META_KEY_PREFIX, this);
        } catch (NacosException e) {
            Loggers.SRV_LOG.error("listen for provider meta change failed!");
        }
    }
    ...
    public void addUpdatedService2Queue(String namespaceId, String serviceName, String serverIP, String checksum) {
        ...
        try {
            ...
            Loggers.SRV_LOG.error("[DOMAIN-STATUS] Failed to add provider to be updatd to queue.", e);
        } finally {
            lock.unlock();
        }
    }
    ...
    public void onChange(String key, Service service) throws Exception {
        try {
            ...
            Loggers.SRV_LOG.error("[NACOS-SERVICE] error while processing provider update", e);
        }
    }
    ...
    public void onDelete(String key) throws Exception {
        ...
        if (service != null && !service.allIPs().isEmpty()) {
            Loggers.SRV_LOG.warn("provider not empty, key: {}", key);
            return;
        }
    ...
    private class UpdatedServiceProcessor implements Runnable {
        //get changed provider from other server asynchronously
        @Override
        public void run() {
            ...
            try {
                ...
                Loggers.EVT_LOG.error("[UPDATE-DOMAIN] Exception while update provider: {}", serviceKey, e);
            }
        }
    }
    ...
    private class ServiceUpdater implements Runnable {
        ...
        public void run() {
            try {
                ...
                Loggers.SRV_LOG.warn("[DOMAIN-UPDATER] Exception while update provider: {} from {}, error: {}",
    ...
    public void updatedHealthStatus(String namespaceId, String serviceName, String serverIP) {
        ...
        Loggers.EVT_LOG.info("[IP-UPDATED] namespace: {}, provider: {}, ips: {}",
    ...
    public void easyRemoveService(String namespaceId, String serviceName) throws Exception {
        ...
        Service service = getService(namespaceId, serviceName);
        if (service == null) {
            throw new IllegalArgumentException("specified provider not exist, serviceName : " + serviceName);
        }

        if (!service.allIPs().isEmpty()) {
            throw new IllegalArgumentException("specified provider has instances, serviceName : " + serviceName);
        }
    ...
    public void createEmptyService(String namespaceId, String serviceName, boolean local) throws NacosException {
        ...
        if (service == null) {
            ...
            Loggers.SRV_LOG.info("creating empty provider {}:{}", namespaceId, serviceName);
            service = new Service();
            service.setName(serviceName);
            service.setNamespaceId(namespaceId);
            service.setGroupName(Constants.DEFAULT_GROUP);
            // now validate the provider. if failed, exception will be thrown
    ...
    /**
     * Register an instance to a provider in AP mode.
     * <p>
     * This method creates provider or cluster silently if they don't exist.
     *
     * @param namespaceId id of namespace
     * @param serviceName provider name
    ...
    public void registerInstance(String namespaceId, String serviceName, Instance instance) throws NacosException {
        ...
        if (service == null) {
            throw new NacosException(NacosException.INVALID_PARAM,
                "provider not found, namespace: " + namespaceId + ", provider: " + serviceName);
        }
    ...
    public List<Instance> updateIpAddresses(Service service, String action, boolean ephemeral, Instance... ips) throws NacosException {
        ...
        if (instanceMap.size() <= 0 && UtilsAndCommons.UPDATE_INSTANCE_ACTION_ADD.equals(action)) {
            throw new IllegalArgumentException("ip list can not be empty, provider: " + service.getName() + ", ip list: "
    ...
    private class ServiceReporter implements Runnable {
        ...
        public void run() {
            try {
                ...
                Loggers.SRV_LOG.error("[DOMAIN-STATUS] Exception while sending provider status", e);
            } finally {
