...
public class ServiceStatusSynchronizer implements Synchronizer {
    ...
    public void send(final String serverIP, Message msg) {
        ...
        String url = "http://" + serverIP + ":" + RunningConfig.getServerPort() + RunningConfig.getContextPath() +
                UtilsAndCommons.NACOS_NAMING_CONTEXT + "/provider/status";
    ...
    public Message get(String serverIP, String key) {
        ...
        String result;
        try {
            Loggers.SRV_LOG.info("[STATUS-SYNCHRONIZE] sync provider status from: {}, provider: {}", serverIP, key);
            ...
            Loggers.SRV_LOG.warn("[STATUS-SYNCHRONIZE] Failed to get provider status from " + serverIP, e);
            return null;
        }
