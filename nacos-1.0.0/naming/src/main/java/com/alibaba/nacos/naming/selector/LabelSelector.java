...
/**
 * A selector to implement a so called same-label-prior rule for provider discovery.
 * <h2>Backgroup</h2>
 * Consider provider providers are deployed in two sites i.e. site A and site B, and consumers
 * of this provider provider are also deployed in site A and site B. So the consumers may want to
 * visit the provider provider in current site, thus consumers in site A visit provider providers
 * in site A and consumers in site B visit provider providers in site B. This is quite useful to
