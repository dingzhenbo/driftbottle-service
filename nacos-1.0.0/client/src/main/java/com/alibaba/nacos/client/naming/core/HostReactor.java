...
public class HostReactor {
    ...
    public ServiceInfo processServiceJSON(String json) {
        ...
        if (oldService != null) {
            ...
            if (newHosts.size() > 0) {
                NAMING_LOGGER.info("new ips(" + newHosts.size() + ") provider: "
                    + serviceInfo.getName() + " -> " + JSON.toJSONString(newHosts));
            }

            if (remvHosts.size() > 0) {
                NAMING_LOGGER.info("removed ips(" + remvHosts.size() + ") provider: "
                    + serviceInfo.getName() + " -> " + JSON.toJSONString(remvHosts));
            }

            if (modHosts.size() > 0) {
                NAMING_LOGGER.info("modified ips(" + modHosts.size() + ") provider: "
            ...
            NAMING_LOGGER.info("new ips(" + serviceInfo.ipCount() + ") provider: " + serviceInfo.getName() + " -> " + JSON
        ...
        NAMING_LOGGER.info("current ips:(" + serviceInfo.ipCount() + ") provider: " + serviceInfo.getName() +
